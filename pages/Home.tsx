  ...
  return (
    <div className="p-6 md:p-10 space-y-10 pb-40">
      <header className="space-y-1">
        <h1 className="text-3xl font-bold tracking-tight text-primary">现在就听</h1>
        <p className="text-secondary font-medium">今日为你推荐</p>
      </header>
      ...
      <section className="relative h-64 md:h-80 w-full rounded-2xl overflow-hidden group">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent flex flex-col justify-end p-8">
          ...
          <h2 className="text-3xl md:text-5xl font-black mb-4 text-primary">流行乐精选</h2>
          <p className="text-secondary max-w-lg mb-6 line-clamp-2">聆听定义当下潮流的旋律，汇聚你最爱艺人的大热单曲。</p>
      ...
      <section>
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-bold tracking-tight text-primary">推荐歌单</h3>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
          {PLAYLISTS.map((playlist) => (
            <div key={playlist.id} className="group cursor-pointer">
              ...
              <p className="text-xs text-tertiary truncate">{playlist.description}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section>
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-bold tracking-tight text-primary">新歌速递</h3>
          <button className="text-rose-500 text-sm font-semibold hover:underline">查看全部</button>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-10 gap-y-4">
          {SONGS.map((song) => (
            <div
              ...
              className="flex items-center gap-4 p-3 rounded-xl hover:bg-card cursor-pointer transition-colors group"
            >
              <img src={song.cover} className="w-16 h-16 rounded-lg object-cover shadow-md" />
              <div className="flex-1 min-w-0">
                <h4 className="font-bold text-sm truncate group-hover:text-rose-500 transition-colors text-primary">{song.title}</h4>
                <p className="text-xs text-tertiary truncate font-medium">{song.artist}</p>
              </div>
